...
- `profile-detection.schema.json` — ranked supplier profile detection result, normalized
  fingerprint, weighted score explanation and blocking issues. Examples cover success,
  ambiguity, `TEMPLATE_CHANGED` and an unknown supplier. `HIGH` confidence is valid only
  when `selectedProfile` is present.
