...
SNAK · АВТОМАТИЗАЦИЯ ЗАКАЗОВ И ПРИЕМКИ ТОВАРА
:::
...
- принятого решения использовать подтвержденные связи «позиция поставщика - номенклатура SNAK» не только при заказе, но и при приемке поставки.
...
3. Для каждой позиции сотрудник вручную находит и пишет внутренний артикул SNAK на бумажной накладной.
...
В колонке «Код вида товара» сотрудники вручную вписали внутренние артикулы SNAK. Именно эта ручная операция затем используется товароведом для повторного создания документа в 1С.
...
|---|---|
| Позиция уже связана с номенклатурой SNAK | Внутренняя номенклатура и упаковка подставляются автоматически. |
