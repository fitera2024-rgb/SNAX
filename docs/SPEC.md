...
**Программа v3.0.** Этот документ остаётся техническим контрактом сервиса импорта. Корпоративная топология, выгрузки баз, каталоги MDM/расширений/обменов и пересчитанный календарь — [SPEC_PROGRAM.md](SPEC_PROGRAM.md), [ADR-004](../adr/ADR-004-forus-topology.md), [SCHEDULE.md](SCHEDULE.md).
:::
