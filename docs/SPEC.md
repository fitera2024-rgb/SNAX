...
SNAK · АВТОМАТИЗАЦИЯ ЗАКАЗОВ ПОСТАВЩИКАМ
:::
...
| Регистр | Назначение |
|---|---|
| `SNAKВнешниеПозицииПоставщиков` | Стабильная карточка внешнего варианта и последние реквизиты |
| `SNAKСоответствияНоменклатуры` | Версионированная связь с номенклатурой/характеристикой/упаковкой 1С |
| `SNAKЦеновыеУровниПоставщиков` | Несколько цен, пороги, валюта, период действия, источник |
| `SNAKДоступностьПоставщиков` | Количество, качественный статус, дата ожидания, время снимка |
| `SNAKПакетыИнтеграции` | Идемпотентность, chunk progress, ACK, ошибки |
| `SNAKИсторияРешенийКМ` | Действия, причины и предыдущие значения, если стандартного журнала недостаточно |

### Перечисления

- `SNAKСтатусПакета`;
- `SNAKСтатусСопоставления`;
- `SNAKСтатусНаличияПоставщика`;
- `SNAKSeverityОшибки`;
- `SNAKТипЦеныПоставщика`.
