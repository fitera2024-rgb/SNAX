...
**«SNAK — централизованная автоматизация заказов поставщикам»**.
...
2. **Расширение 1С:Управление торговлей** — принимает пакет во временный контур, сопоставляет внешние позиции с номенклатурой SNAK, хранит подтверждённые связи, применяет цены и закупочные параметры, после чего использует типовой механизм обеспечения потребностей и формирования заказов поставщикам.
...
| ID | Требование | Приоритет |
|---|---|---|
| FR-1C-001 | Расширение создаёт объект «Импорт прайс-листа SNAK» или эквивалентный staging-документ. | Must |
...
| FR-1C-020 | Остаток/наличие поставщика хранится отдельно и не увеличивает складской остаток SNAK. | Must |
